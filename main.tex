...
\begin{document}
	...
	\subsection{为何选择这四个模型}
	四个 baseline 的选择兼顾\textbf{范式多样性}与\textbf{核心对照轴}：它们分别代表三分图精修、SAM 式零样本可提示、扩散式生成先验、自动与交互统一四条不同技术路线，数量满足课程要求；更重要的是，ViTMatte 依赖外部三分图、其余三者自主确定前景，恰好构成「给定前景的精修 vs 自主前景选择」这一最干净的对照，便于在植物复杂场景中分别考察「语义理解/目标选择」与「局部边缘细化」两类能力，并据此展开 failure case 与认知分析。
